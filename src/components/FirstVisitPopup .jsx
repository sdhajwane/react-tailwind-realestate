...
const FirstVisitPopup = ({ setShowPopup }) => {
  const [showPopup, setShowPopupState] = useState(false);

  // We handle the 2-second delay only for the first visit, not for brochure click
  useEffect(() => {
  ...
  const handleClose = () => {
    setShowPopupState(false); // Close the popup when the close button is clicked
    setShowPopup(false); // Close the popup from the parent (Navbar)
  };

  if (!showPopup) return null; // Don't render popup if it's not active
